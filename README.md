# Task Manager API

RESTful API приложение для управления задачами, построенное на Laravel 12 с использованием PHP 8.4 и PostgreSQL.

## 📋 Описание

Task Manager - это полнофункциональное API приложение для управления задачами с поддержкой:
- CRUD операций для задач
- Системы уведомлений
- Аутентификации и авторизации
- Админ-панели со статистикой
- Шифрования чувствительных данных
- Генерации мотивационных сообщений с помощью LLM (Ollama)

## 🛠 Технологический стек

- **PHP 8.4** - Язык программирования
- **Laravel 12** - PHP фреймворк
- **PostgreSQL 15** - База данных
- **Redis 7** - Кэширование и очереди
- **Ollama** - Локальная LLM для генерации мотивационных сообщений
- **Docker** - Контейнеризация
- **Nginx** - Веб-сервер
- **Pest** - Фреймворк для тестирования

## 🏗 Архитектура

Проект построен с использованием следующих принципов и подходов:

- **DDD (Domain-Driven Design)** - Структура организована по доменам (Task, User, Notification, Motivation)
- **SOLID** - Принципы объектно-ориентированного программирования
- **DRY** - Избежание дублирования кода
- **KISS** - Простота и понятность кода
- **TDD** - Разработка через тестирование
- **Clean Code** - Чистый и поддерживаемый код

## 📁 Структура проекта

```
app/
├── Domain/              # Доменная логика (DDD)
│   ├── Common/         # Общие компоненты
│   ├── Task/           # Домен задач
│   ├── User/           # Домен пользователей
│   ├── Notification/   # Домен уведомлений
│   └── Motivation/     # Домен мотивационных сообщений
│       ├── Contracts/  # Интерфейсы провайдеров
│       ├── Enums/      # Типы сообщений
│       ├── Providers/  # Провайдеры (Ollama)
│       └── Services/   # Сервисы генерации
├── Http/
│   ├── Controllers/    # Контроллеры
│   ├── Requests/       # Валидация запросов
│   ├── Resources/      # API Resources
│   ├── Middleware/     # Middleware
│   └── Rules/          # Кастомные правила валидации
├── Policies/           # Политики авторизации
└── Providers/          # Service Providers
```

## 🚀 Установка и развертывание

### Требования

- Docker и Docker Compose
- Git
- Make (опционально, для использования Makefile)

### Быстрая установка

1. Клонируйте репозиторий:
```bash
git clone <repository-url>
cd crud-laravel
```

2. Запустите установку одним из способов:

#### Вариант 1: Использование Make (рекомендуется)
```bash
make build
```

#### Вариант 2: Использование bash скрипта
```bash
chmod +x build.sh
./build.sh
```

Скрипт автоматически:
- Создаст `.env` файл из `.env.example`
- Соберет и запустит Docker контейнеры
- Установит зависимости Composer
- Выполнит миграции базы данных
- Сгенерирует ключ приложения
- Загрузит модель Ollama (llama3.2) для генерации мотивационных сообщений

### Полная пересборка

Для полной пересборки проекта (удаление всех данных):

#### Вариант 1: Использование Make
```bash
make rebuild
```

#### Вариант 2: Использование bash скрипта
```bash
chmod +x rebuild.sh
./rebuild.sh
```

### Дополнительные команды Make

После установки доступны следующие команды:

```bash
make help         # Показать справку по всем командам
make up           # Запустить контейнеры
make down         # Остановить контейнеры
make test         # Запустить тесты
make logs         # Показать логи приложения
make shell        # Войти в контейнер приложения
make migrate      # Выполнить миграции
make fresh        # Выполнить свежие миграции (с удалением данных)
make clean        # Очистить кэш
make cache-clear  # Очистить весь кэш (включая Redis)
make ollama-setup # Настроить Ollama (загрузить модель)
```

### Проверка работоспособности

После успешной установки проект будет доступен по адресу `http://localhost:8080`.

Для проверки работоспособности API можно выполнить:

1. **Регистрация пользователя:**
```bash
curl -X POST http://localhost:8080/api/register \
  -H "Content-Type: application/json" \
  -d '{
    "name": "Test User",
    "email": "test@example.com",
    "password": "password123",
    "password_confirmation": "password123"
  }'
```

2. **Вход в систему (получение токена):**
```bash
curl -X POST http://localhost:8080/api/login \
  -H "Content-Type: application/json" \
  -d '{
    "email": "test@example.com",
    "password": "password123"
  }'
```

В ответе вы получите токен и мотивационное сообщение:
```json
{
  "data": {
    "user": {...},
    "token": "your-token-here",
    "motivation_message": "Сегодня отличный день для новых достижений!"
  }
}
```

Токен можно использовать для доступа к защищенным эндпоинтам:

```bash
curl -X GET http://localhost:8080/api/me \
  -H "Authorization: Bearer YOUR_TOKEN_HERE"
```

## 🧪 Запуск тестов

Проект использует **Pest** для тестирования. Тесты используют ту же базу данных PostgreSQL, что и основное приложение.

Для запуска тестов используйте:

#### Вариант 1: Использование Make (рекомендуется)
```bash
make test
```

#### Вариант 2: Использование bash скрипта
```bash
chmod +x test.sh
./test.sh
```

#### Вариант 3: Напрямую через Docker
```bash
docker-compose exec app php artisan test
```

Проект включает:
- **Feature тесты** - Тестирование API endpoints (включая проверку мотивационных сообщений)
- **Unit тесты** - Тестирование сервисов и бизнес-логики (включая MotivationService)

## 📡 API Endpoints

Все API endpoints возвращают JSON. Для аутентификации используется **Laravel Sanctum** с токенами.

### Аутентификация

Для доступа к защищенным маршрутам необходимо:
1. Зарегистрироваться или войти в систему
2. Получить токен из ответа
3. Передавать токен в заголовке `Authorization: Bearer {token}`

### Публичные маршруты

- `POST /api/register` - Регистрация пользователя
  ```json
  {
    "name": "John Doe",
    "email": "john@example.com",
    "password": "password123",
    "password_confirmation": "password123"
  }
  ```
  **Ответ:** Возвращает `user`, `token` и `motivation_message` (уникальное мотивационное сообщение, сгенерированное LLM)

- `POST /api/login` - Вход в систему
  ```json
  {
    "email": "john@example.com",
    "password": "password123"
  }
  ```
  **Ответ:** Возвращает `user`, `token` и `motivation_message` (уникальное мотивационное сообщение, сгенерированное LLM)

### Защищенные маршруты (требуют аутентификации)

#### Аутентификация
- `POST /api/logout` - Выйти из системы
- `GET /api/me` - Получить информацию о текущем аутентифицированном пользователе

#### Уведомления
- `GET /api/notifications` - Получить список уведомлений (с пагинацией)
- `POST /api/notifications` - Создать новое уведомление
- `PATCH /api/notifications/{id}/read` - Отметить уведомление как прочитанное
- `PATCH /api/notifications/mark-all-read` - Отметить все уведомления как прочитанные
- `GET /api/notifications/unread-count` - Получить количество непрочитанных уведомлений

### Админ-маршруты (требуют роль admin)

#### Задачи
- `GET /api/admin/tasks` - Получить список всех задач (с пагинацией)
- `POST /api/admin/tasks` - Создать новую задачу
- `GET /api/admin/tasks/{id}` - Получить информацию о задаче
- `PUT /api/admin/tasks/{id}` - Обновить задачу
- `DELETE /api/admin/tasks/{id}` - Удалить задачу
- `PATCH /api/admin/tasks/{id}/completed` - Отметить задачу как завершенную
- `PATCH /api/admin/tasks/{id}/in-progress` - Отметить задачу как в работе
- `PATCH /api/admin/tasks/{id}/pending` - Отметить задачу как в ожидании
- `PATCH /api/admin/tasks/{id}/assign` - Назначить задачу пользователю

#### Статистика
- `GET /api/admin/dashboard/statistics` - Статистика для админ-панели

## 🔐 Безопасность

- **XSS защита** - Валидация всех пользовательских данных на XSS атаки
- **Шифрование ID** - Чувствительные ID шифруются при передаче клиенту
- **Sanctum аутентификация** - Token-based аутентификация
- **Роли пользователей** - Поддержка ролей (admin, manager, user)
- **Валидация запросов** - Все запросы валидируются через Form Requests

## 📊 База данных

Проект использует PostgreSQL со следующими основными таблицами:
- `users` - Пользователи
- `tasks` - Задачи
- `notifications` - Уведомления
- `sessions` - Сессии
- `cache` - Кэш (используется только если CACHE_STORE=database)
- `jobs` - Очереди задач

**Redis** используется для:
- Кэширования данных (статистика задач кэшируется на 5 минут)
- Кэширования конфигурации, роутов и представлений Laravel
- Улучшения производительности приложения

### Очистка кэша

Для очистки всего кэша приложения (включая Redis) используйте команду:

```bash
# Через Docker
docker-compose exec app php artisan cache:clear-all

# Через Make
make cache-clear

# Или стандартные команды Laravel
docker-compose exec app php artisan cache:clear
```

**Примечание:** Кэш статистики задач автоматически очищается при любых изменениях задач (создание, обновление, удаление, изменение статуса, назначение пользователю).

## 📝 Особенности реализации

- **DTO (Data Transfer Objects)** - Использование DTO для передачи данных между слоями
- **Enums** - PHP Enums для статусов, приоритетов, типов уведомлений и мотивационных сообщений
- **API Resources** - Трансформация моделей для API ответов
- **Form Requests** - Валидация запросов
- **Service Layer** - Бизнес-логика в сервисах
- **Policies** - Авторизация через политики

## 🐳 Docker

Проект включает следующие сервисы:
- **app** - PHP-FPM приложение (порт 9000, внутренний)
- **nginx** - Веб-сервер (порт 8080, внешний)
- **db** - PostgreSQL 15 (порт 5432, внешний)
- **redis** - Redis 7 (порт 6379, внешний)
- **ollama** - LLM сервер для генерации мотивационных сообщений (порт 11434, внешний)

Все сервисы работают в единой Docker сети `task-manager-network`.

### Настройка Ollama

После первого запуска проекта модель Ollama загружается автоматически. Если по какой-то причине модель не загрузилась, выполните:

```bash
make ollama-setup
```

Или напрямую через Docker:

```bash
docker-compose exec ollama ollama pull llama3.2
```

## 🤖 Генерация мотивационных сообщений

Приложение использует локальную LLM (Ollama) для генерации уникальных мотивационных сообщений при авторизации пользователей.

### Особенности

- **Типы сообщений:** Мотивационные фразы, шутки, интересные факты, вдохновляющие цитаты
- **Уникальность:** Каждое сообщение генерируется заново при каждой авторизации
- **Fallback система:** При недоступности Ollama используются предустановленные сообщения
- **Креативность:** Промпты настроены на генерацию необычных, креативных сообщений с элементами юмора и иронии

### Конфигурация

Настройки Ollama находятся в `.env`:

```env
MOTIVATION_PROVIDER=ollama
OLLAMA_URL=http://ollama:11434
OLLAMA_MODEL=llama3.2
```

### Как это работает

1. При успешной регистрации или входе система случайным образом выбирает тип сообщения
2. Отправляется запрос в Ollama с промптом для генерации сообщения
3. Если Ollama недоступна, используется fallback с предустановленными сообщениями
4. Сгенерированное сообщение возвращается в ответе API вместе с данными пользователя и токеном

## 📚 Дополнительная информация

- Все контроллеры используют централизованную обработку ошибок через базовый `Controller`
- Все ответы API стандартизированы (успешные и ошибочные)
- Код полностью покрыт Feature и Unit тестами
- Соблюдены PSR стандарты кодирования
- Используется кастомная валидация для защиты от XSS атак
- ID пользователей и задач шифруются при передаче клиенту
- Тесты используют ту же базу данных PostgreSQL, что и основное приложение

## 👥 Роли пользователей

- **admin** - Полный доступ ко всем функциям
- **manager** - Расширенные права
- **user** - Базовые права

## 📞 Поддержка

Для вопросов и предложений создайте issue в репозитории.

## 📄 Лицензия

Проект создан в целях практики проектирования Laravel-приложений.

